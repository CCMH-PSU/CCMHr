...
ccaps62_cuts <- function(data, version, first = F){
    ...
    stop('CCAPS columns not named correctly.')
      ...
      if(first == F) {
        if(version == "2018") {
          ...
          data <- data %>%
            dplyr::mutate(depression_low_cut62 = ifelse(round(.data$Depression62, 2) >= 1.23, 1, 0),
                          depression_hi_cut62 = ifelse(round(.data$Depression62, 2) >= 1.92, 1, 0),
                          anxiety_low_cut62 = ifelse(round(.data$Anxiety62, 2) >= 1.22, 1, 0),
                          anxiety_hi_cut62 = ifelse(round(.data$Anxiety62,2) >= 1.89, 1, 0),
                          social_anxiety_low_cut62 = ifelse(round(.data$Social_Anxiety62, 2) >= 1.43, 1, 0),
                          social_anxiety_hi_cut62 = ifelse(round(.data$Social_Anxiety62,2) >= 2.57, 1, 0),
                          academics_low_cut62 = ifelse(round(.data$Academics62,2) >= 1.2, 1, 0),
                          academics_hi_cut62 = ifelse(round(.data$Academics62,2) >= 2.40, 1, 0),
                          eating_low_cut62 = ifelse(round(.data$Eating62,2) >= 1.02, 1, 0),
                          eating_hi_cut62 = ifelse(round(.data$Eating62,2) >= 1.80, 1, 0),
                          hostility_low_cut62 = ifelse(round(.data$Hostility62,2) >= 1, 1, 0),
                          hostility_hi_cut62 = ifelse(round(.data$Hostility62,2) >= 1.43, 1, 0),
                          substance_low_cut62 = ifelse(round(.data$Substance62,2) >= .69, 1, 0),
                          substance_hi_cut62 = ifelse(round(.data$Substance62,2) >= 1, 1, 0),
                          family_low_cut62 = ifelse(round(.data$Family62,2) >= 1.31, 1, 0),
                          family_hi_cut62 = ifelse(round(.data$Family62,2) >= 1.83, 1, 0)
              )
            return(data)
        ...
        if(version == "2018") {
          data <- data %>%
            dplyr::mutate(depression_low_cut62 = ifelse(round(.data$Depression62_first, 2) >= 1.09, 1, 0),
                          depression_hi_cut62 = ifelse(round(.data$Depression62_first, 2) >= 1.92, 1, 0),
                          anxiety_low_cut62 = ifelse(round(.data$Anxiety62_first, 2) >= 1.25, 1, 0),
                          anxiety_hi_cut62 = ifelse(round(.data$Anxiety62_first,2) >= 1.89, 1, 0),
                          social_anxiety_low_cut62 = ifelse(round(.data$Social_Anxiety62_first, 2) >= 1.72, 1, 0),
                          social_anxiety_hi_cut62 = ifelse(round(.data$Social_Anxiety62_first,2) >= 2.57, 1, 0),
                          academics_low_cut62 = ifelse(round(.data$Academics62_first,2) >= 1.42, 1, 0),
                          academics_hi_cut62 = ifelse(round(.data$Academics62_first,2) >= 2.40, 1, 0),
                          eating_low_cut62 = ifelse(round(.data$Eating62_first,2) >= 1.09, 1, 0),
                          eating_hi_cut62 = ifelse(round(.data$Eating62_first,2) >= 1.80, 1, 0),
                          hostility_low_cut62 = ifelse(round(.data$Hostility62_first,2) >= .82, 1, 0),
                          hostility_hi_cut62 = ifelse(round(.data$Hostility62_first,2) >= 1.43, 1, 0),
                          substance_low_cut62 = ifelse(round(.data$Substance62_first,2) >= .7, 1, 0),
                          substance_hi_cut62 = ifelse(round(.data$Substance62_first,2) >= 1, 1, 0),
                          family_low_cut62 = ifelse(round(.data$Family62_first,2) >= .98, 1, 0),
                          family_hi_cut62 = ifelse(round(.data$Family62_first,2) >= 1.83, 1, 0)
            )
          return(data)
          message("CCAPS-62 low cut scores based on original 2012 norms and high cut scores based on 2018 norms.")
        } else if (version == "2019") {
          data <- data %>%
            dplyr::mutate(depression_low_cut62 = ifelse(round(.data$Depression62_first, 2) >= 1.23, 1, 0),
                          depression_hi_cut62 = ifelse(round(.data$Depression62_first, 2) >= 1.92, 1, 0),
                          anxiety_low_cut62 = ifelse(round(.data$Anxiety62_first, 2) >= 1.22, 1, 0),
                          anxiety_hi_cut62 = ifelse(round(.data$Anxiety62_first,2) >= 1.89, 1, 0),
                          social_anxiety_low_cut62 = ifelse(round(.data$Social_Anxiety62_first, 2) >= 1.43, 1, 0),
                          social_anxiety_hi_cut62 = ifelse(round(.data$Social_Anxiety62_first,2) >= 2.57, 1, 0),
                          academics_low_cut62 = ifelse(round(.data$Academics62_first,2) >= 1.2, 1, 0),
                          academics_hi_cut62 = ifelse(round(.data$Academics62_first,2) >= 2.40, 1, 0),
                          eating_low_cut62 = ifelse(round(.data$Eating62_first,2) >= 1.02, 1, 0),
                          eating_hi_cut62 = ifelse(round(.data$Eating62_first,2) >= 1.80, 1, 0),
                          hostility_low_cut62 = ifelse(round(.data$Hostility62_first,2) >= 1, 1, 0),
                          hostility_hi_cut62 = ifelse(round(.data$Hostility62_first,2) >= 1.43, 1, 0),
                          substance_low_cut62 = ifelse(round(.data$Substance62_first,2) >= .69, 1, 0),
                          substance_hi_cut62 = ifelse(round(.data$Substance62_first,2) >= 1, 1, 0),
                          family_low_cut62 = ifelse(round(.data$Family62_first,2) >= 1.31, 1, 0),
                          family_hi_cut62 = ifelse(round(.data$Family62_first,2) >= 1.83, 1, 0)
            )
          return(data)
          message("CCAPS-62 low cut scores based on 2019 norms and high cut scores based on 2018 norms.")
