...
#' @name plot_bar
#'
#' @description This function creates a bar graph. Bar graphs visually resemble column graphs; however, there are critical differences. In ggplot2 bar graphs, the height of bars is proportional to the number of responses at each x-axis position (e.g., what is the number of clients that responded 1, 2, 3, 4, or 5). Here, ggplot2 uses stat_count() by default, where it counts the number of responses at each x-axis position. Alternatively, ggplot2 column plots display data that are already summarized (e.g., a table containing the count or prevalence of responses). Thus, the height of the bars is based on the numbers within the summarized data (e.g., 123 responded with 1, 178 responded with 2, 332 responded with 3, 472 responded with 4, or 986 responded with 5). See the example data frame below for reference.
#'
...
#' @param group.var A quoted string to indicate the variable that will group data/bars on the x-axis. By default, `""`.
#' @param color A hex code or list of hex codes that indicates the color(s) of the bars. If a variable groups the data, the argument specifies the color of the bars or items within the grouped variable. For example, the group variable items could be the names of automobile companies (i.e., Ford, Dodge, BMW). You can specify a random color for each company using this list: `c("#21501b", "#c51329", "#074e67")`. Alternatively, you can specify the color directly for each company using this list: `c("Ford" = "#21501b", "Dodge" = "#c51329", "BMW" = "#074e67")`. The number of hex codes must be equal to or greater than the number of responses in the grouped data. If data is not grouped, all the bars will have the same color. The default colors are color-blind safe. By default, `CCMHr::CCMH_categorical_palettes$tolmuted_category10`.
#' @param y.type A quoted string to indicate if the information on the y-axis concerns the count or percentage of the x-axis and/or group variables. Options include `"count"` or `"percent"`. By default, `"count"`.
#' @param x.order.manual A list of quoted strings to indicate the order of the x-axis items. By default, `NULL`.
#' @param col.position A quoted string to indicate the bar position adjustment. By default, `"dodge"`.
...
#' @param x.remove A logical argument to indicate whether the y-axis information should be removed. If `TRUE`, the y-axis title, labels, and ticks will be removed. By default, `FALSE`.
