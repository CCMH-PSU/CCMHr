...
#' @param x.var A quoted string to indicate the variable that will be plotted on the x-axis. The variable class must be numeric.
...
#' @param color A hex code or list of hex codes that indicates the color of the grouped points. See Note 1 for more details. By default, `"#838B8B"` or a shade of dark grey.
...
#' @param x.label.percent A logical statement to indicate whether the x axis labels are percentages. By default, `FALSE`.
...
scatter_plot <- function(data,
  ...
  # If x-axis is a percentage
    {if(x.label.percent == TRUE){
      ...
      ggplot2::scale_x_continuous(limits = c(x.min,
                                             x.max),
                                  expand = c(0,0))
